...
func trustOnFirstUseCallback() (gossh.HostKeyCallback, error) {
	...
	return func(hostname string, remote net.Addr, key gossh.PublicKey) error {
		...
		if len(keyErr.Want) > 0 {
			...
			return fmt.Errorf("SSH host key for %s doesn't match the one on record in %s (possible MITM, or the host was reprovisioned) — remove its entry from that file to re-trust it", hostname, knownHostsPath)
		}

		// Never seen this host before: record its key and accept.
		if appendErr := appendKnownHost(hostname, remote, key); appendErr != nil {
...
func appendKnownHost(hostname string, remote net.Addr, key gossh.PublicKey) error {
